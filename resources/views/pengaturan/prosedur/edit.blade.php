@extends('layouts.app')
@section('title', 'Edit Data Prosedur')

@section('content')
<div class="container">
    <form method="POST" action="{{ route('prosedur.update', $prosedur->id) }}">
        @csrf
        @method('PUT')
        <div class="form-group mb-3">
            <label class="form-label">Urutan <span class="text-danger">*</span></label>
            <input type="number" id="urut" name="urut" class="form-control @error('urut') is-invalid @enderror" 
                   placeholder="Masukkan nomor urut" value="{{ old('urut', $prosedur->urut) }}">
            @error('urut')
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" id="judul" name="judul" class="form-control @error('judul') is-invalid @enderror" 
                   placeholder="Masukkan judul prosedur" value="{{ old('judul', $prosedur->judul) }}">
            @error('judul')
              <div class="invalid-feedback">
                {{ $message }}
              </div>
        ...
        <div class="form-group mb-3">
            ...
            <textarea id="deskripsi" name="deskripsi" 
                      ...
                      rows="4">{{ old('deskripsi', $prosedur->deskripsi) }}</textarea>
            @error('deskripsi')
              <div class="invalid-feedback">
                {{ $message }}
              </div>
